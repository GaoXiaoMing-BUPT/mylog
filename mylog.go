...
func init() {
	...
	logFile.level = WarnLevel
	logFile.filesize = 1024 * 1024 // 默认1M  单位byte， 需要设置 saveMode 为 BySize
}

func Config(logFolder string, level int) {
	logFile.fileName = logFolder
	logFile.level = level
	if strings.EqualFold(logFolder, "os.Stdout") {
		// 实现write接口
		log.SetOutput(os.Stdout)
	} else {
		log.SetOutput(logFile)
		if colorFul {
			Warnf("please be sure colorFul is false other than windows")
		}
	}
...
func (me LogFile) Write(buf []byte) (n int, err error) {
	...
	case BySize:
		...
		if err != nil {
			...
			if logFile.fileFd == nil || filesize > logFile.filesize {
				logFile.createLogFile()
				logFile.logTime = time.Now().Unix()
			}
		}
